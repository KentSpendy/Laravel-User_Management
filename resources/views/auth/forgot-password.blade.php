...
<head>
    ...
    <title>FORCE RECOVERY | GALACTIC COMMAND</title>
    <style>
...
<body>
    ...
    <div class="force-container">
        ...
        <div style="font-size: 0.9rem; color: var(--force-light); text-align: center; margin-top: 0.5rem; letter-spacing: 1px;">
            JEDI-SITH SECURITY PROTOCOL
        </div>
        
        <form method="POST" action="{{ route('password.email') }}" style="display: flex; flex-direction: column; gap: 1.5rem;">
            @csrf
            <div class="form-group">
                <label><i class="fas fa-envelope"></i> HOLONET CONTACT:</label>
                <i class="fas fa-envelope input-icon"></i>
                <input type="email" name="email" placeholder="ENTER YOUR HOLONET ID" required>
            </div>
            
            <button type="submit">
                <i class="fas fa-unlock-alt"></i> INITIATE FORCE RECOVERY
            </button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            input.addEventListener('focus', function() {
                icon.style.color = 'var(--lightsaber-blue)';
                icon.style.textShadow = '0 0 10px var(--lightsaber-blue)';
            });
